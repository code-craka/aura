...
class AuraTabManager extends events_1.EventEmitter {
    constructor(processManager, config) {
        ...
        this.bookmarkedTabs = new Map(); // tabId -> folder
    ...
    async createTab(url, options = {}) {
        ...
        const processInfo = await this.processManager.createProcess(process_manager_1.ProcessType.Renderer, {
            memoryLimit: 100 * 1024 * 1024, // 100MB default
            securityLevel: process_manager_1.SecurityLevel.Standard,
        ...
        const tab = {
            ...
            url,
            title: 'Loading...',
            status: chromium_engine_1.TabStatus.Loading,
            groupId,
            spaceId,
            suspended: false,
            lastActive: new Date(),
    ...
    async moveTab(tabId, targetGroupId) {
        const tab = this.tabs.get(tabId);
        if (!tab) {
            throw new Error(`Tab ${tabId} not found`);
        }
    ...
    async suspendTab(tabId) {
        ...
        try {
            ...
            // Mark as suspended
            tab.suspended = true;
            tab.status = chromium_engine_1.TabStatus.Suspended;
            this.tabs.set(tabId, tab);
    ...
    async restoreTab(tabId) {
        ...
        try {
            ...
            // Mark as restored
            tab.suspended = false;
            tab.status = chromium_engine_1.TabStatus.Complete;
    ...
    async createSpace(name, settings = {}) {
        const space = {
    ...
    async optimizeMemory() {
        ...
        const sortedTabs = tabs
            ...
            .sort((a, b) => {
            // Pinned tabs first
            if (this.pinnedTabs.has(a.id) && !this.pinnedTabs.has(b.id))
                return 1;
            if (!this.pinnedTabs.has(a.id) && this.pinnedTabs.has(b.id))
                return -1;
        ...
        if (optimizationStrategy.suspendInactive) {
            ...
            for (const tab of inactiveTabs) {
                if (totalMemory < memoryLimit * 0.9)
                    break; // Stop if memory is below 90%
                try {
        ...
        if (optimizationStrategy.reduceNonEssential) {
            ...
            for (const tab of nonEssentialTabs) {
                if (totalMemory < memoryLimit * 0.85)
                    break; // Stop if memory is below 85%
                try {
        ...
        if (optimizationStrategy.aggressiveSuspension) {
            ...
            for (const tab of remainingTabs) {
                if (totalMemory < memoryLimit * 0.8)
                    break; // Stop if memory is below 80%
                try {
                    await this.suspendTab(tab.id);
                    suspendedCount++;
                    memoryFreed += tab.memoryUsage;
        ...
        for (const [tabId, tab] of this.tabs) {
            ...
            switch (this.config.suspensionStrategy.type) {
                ...
                case 'usage-based':
                    ...
                    if (hasRecentContent)
                        suspensionScore *= 0.5; // Reduce score if recent content
    ...
    async restoreTabState(tab, state) {
    ...
    async searchTabsAdvanced(query) {
        ...
        if (query.dateRange) {
            allTabs = allTabs.filter(tab => tab.createdAt >= query.dateRange.start && tab.createdAt <= query.dateRange.end);
        }
        // Apply sorting
    ...
    async migrateTabsToSpace(tabIds, targetSpaceId) {
    ...
    async duplicateTab(tabId, targetGroupId) {
    ...
    async bookmarkTab(tabId, folder) {
        const tab = this.tabs.get(tabId);
        if (!tab) {
            throw new Error(`Tab ${tabId} not found`);
        }
    ...
    async importSpace(exportData) {
        // Create new space
        const space = await this.createSpace(`${exportData.space.name} (Imported)`, exportData.space.settings);
        // Create groups
...
exports.AuraTabManager = AuraTabManager;
// Factory function
function createTabManager(processManager, config) {
